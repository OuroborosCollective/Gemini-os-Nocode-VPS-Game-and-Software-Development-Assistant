...
export const ArchitectPanel: React.FC = () => {
  ...
  const handleAIAction = async (action: string, sysPrompt: string, options: { isCodeUpdate?: boolean, newExtension?: string, outputFilename?: string, suffix?: string } = {}) => {
    if (!currentFileContent && !options.outputFilename && action !== "Auto-README" && action !== "CI/CD" && action !== "Dockerize") return;

    logToSystem(`✨ <b>Führe ${action} für ${currentFile !== "Keine Datei gewählt" ? currentFile : "Projekt"}... durch</b>`, "info");
    const isFileProducingAction = options.isCodeUpdate || options.newExtension || options.outputFilename || options.suffix;
    if (!isFileProducingAction) setActiveTab("chat");

    try {
      let promptText = `Datei: ${currentFile}\n\nCode:\n${currentFileContent}`;
      if (action === "Auto-README" || action === "CI/CD" || action === "Dockerize") {
          const paths = fullTree.slice(0, 100).map(f => f.path).join('\n');
          promptText = `Dateibaum:\n${paths}\n\nAktuelle Datei (${currentFile}):\n${currentFileContent.substring(0, 1500)}`;
      }

      let response = await callGeminiAPI(promptText, sysPrompt);

      if (isFileProducingAction) {
        ...
        if (options.outputFilename) {
            ...
            if (options.suffix && options.newExtension) {
                targetFile = `${base}${options.suffix}.${options.newExtension}`;
            } else if (options.suffix) {
                targetFile = `${base}${options.suffix}.${ext}`;
            } else if (options.newExtension) {
                targetFile = `${base}.${options.newExtension}`;
            }
        }
  ...
  const handleVoiceExplain = async () => {
    ...
    try {
      ...
      const ttsUrl = `https://generativelanguage.googleapis.com/v1beta/models/${ttsModelName}:generateContent?key=${apiKey}`;
              ...
              {currentFile !== "Keine Datei gewählt" && (
                <>
                  ...
                  <button onClick={() => {
                      ...
                      if (lang) handleAIAction("Translate", `Du bist ein Expert Developer. Übersetze den folgenden Code in ${lang}. Erhalte die exakte Logik bei. Gib NUR den reinen übersetzten Code zurück, ohne Markdown-Blöcke.`, { suffix: "_translated", newExtension: extMap[lang.toLowerCase()] || "txt" });
                  ...
                  <button onClick={() => handleAIAction("Mock Data", "Du bist ein Backend-Entwickler. Analysiere den Code (Modelle, Interfaces, Variablen oder UI-Komponenten) und generiere dazu passende, extrem realistische Mock-Daten als JSON-Array mit 5 detaillierten Objekten. Gib AUSSCHLIESSLICH das nackte JSON zurück, ohne Markdown-Fences drumherum.", { suffix: "_mock", newExtension: "json" })} className="shrink-0 text-[10px] bg-indigo-100 text-indigo-700 px-2 py-1 rounded hover:bg-indigo-200 font-bold shadow-sm transition-all">✨ Mock Data</button>
                  ...
                  <button onClick={() => handleAIAction("i18n", "Du bist ein Frontend-Architekt. Analysiere den Code und extrahiere alle hartcodierten, nutzersichtbaren Texte (Strings). Generiere eine JSON-Datei mit Key-Value-Paaren (Keys in UPPER_SNAKE_CASE, Values sind die Originaltexte). Gib AUSSCHLIESSLICH das validierte JSON zurück, absolut kein Markdown drumherum.", { suffix: "_i18n", newExtension: "json" })} className="shrink-0 text-[10px] bg-yellow-100 text-yellow-700 px-2 py-1 rounded hover:bg-yellow-200 font-bold shadow-sm transition-all">✨ i18n</button>
                  <button onClick={() => handleAIAction("cURL API", "Du bist ein API-Spezialist. Analysiere diesen Code auf API-Endpunkte (z.B. REST-Routen, Controller, Fetch-Aufrufe). Generiere ein strukturiertes Markdown-Dokument mit realistischen, kopierbaren `curl`-Befehlen zum Testen dieser Endpunkte (inklusive nötiger JSON-Bodys und Headers). Gib NUR Markdown zurück, ohne Fences drumherum.", { suffix: "_endpoints", newExtension: "md" })} className="shrink-0 text-[10px] bg-cyan-100 text-cyan-700 px-2 py-1 rounded hover:bg-cyan-200 font-bold shadow-sm transition-all">✨ cURL API</button>
                  ...
                  <button onClick={() => handleAIAction("Dockerize", "Du bist ein DevOps Engineer. Analysiere die Dateipfade des Repositories und den Code der aktuell geöffneten Datei, um den Tech-Stack zu erraten. Generiere ein professionelles, multi-stage, produktionsbereites 'Dockerfile'. Gib AUSSCHLIESSLICH den Inhalt des Dockerfiles zurück, ohne Markdown-Fences drumherum.", { outputFilename: "Dockerfile" })} className="shrink-0 text-[10px] bg-slate-100 text-slate-700 px-2 py-1 rounded hover:bg-slate-200 font-bold shadow-sm transition-all">✨ Dockerize</button>
                  ...
                  <button onClick={() => handleAIAction("SQL Schema", "Du bist ein Database Administrator. Analysiere den Code (Interfaces, Klassen, Structs, ORM-Modelle, JSON-Daten) und generiere das entsprechende SQL-Schema (PostgreSQL) mit CREATE TABLE Statements, korrekten Datentypen, Primary Keys und Foreign Keys. Gib AUSSCHLIESSLICH den rohen SQL-Code zurück, ohne Markdown-Fences drumherum.", { suffix: "_schema", newExtension: "sql" })} className="shrink-0 text-[10px] bg-amber-100 text-amber-700 px-2 py-1 rounded hover:bg-amber-200 font-bold shadow-sm transition-all">✨ SQL Schema</button>
                  <button onClick={() => handleAIAction("TS Types", "Du bist ein TypeScript-Experte. Analysiere das übergebene JSON, JavaScript oder die ungetypten Strukturen und generiere strikte, professionell benannte TypeScript Interfaces und Types dafür. Gib AUSSCHLIESSLICH den rohen TypeScript-Code zurück, ohne Markdown-Fences drumherum.", { suffix: "_types", newExtension: "ts" })} className="shrink-0 text-[10px] bg-lime-100 text-lime-700 px-2 py-1 rounded hover:bg-lime-200 font-bold shadow-sm transition-all">✨ TS Types</button>
                  <button onClick={() => handleAIAction("CI/CD", "Du bist ein DevOps Architekt. Analysiere den Dateibaum, um den Tech-Stack zu erkennen. Schreibe eine professionelle GitHub Actions CI/CD Pipeline (`main.yml`), die den Code auscheckt, Abhängigkeiten installiert, Tests ausführt und das Projekt baut. Gib AUSSCHLIESSLICH den rohen YAML-Code zurück, ohne Markdown-Fences.", { outputFilename: ".github/workflows/main.yml" })} className="shrink-0 text-[10px] bg-rose-100 text-rose-700 px-2 py-1 rounded hover:bg-rose-200 font-bold shadow-sm transition-all">✨ CI/CD</button>
                  <button onClick={() => handleAIAction("Tutorial", "Du bist ein Developer Advocate und Tech-Blogger. Schreibe ein gut strukturiertes Markdown-Tutorial (wie einen Medium- oder Dev.to-Artikel), das den übergebenen Code Schritt für Schritt erklärt. Gliedere in Einleitung, Code-Walkthrough und Fazit. Gib AUSSCHLIESSLICH Markdown-Text zurück, ohne umschließende Fences für die ganze Antwort.", { suffix: "_tutorial", newExtension: "md" })} className="shrink-0 text-[10px] bg-violet-100 text-violet-700 px-2 py-1 rounded hover:bg-violet-200 font-bold shadow-sm transition-all">✨ Tutorial</button>
                  ...
                  <button onClick={() => {
                      const parts = currentFile.split('.');
                      const ext = parts.pop();
                      handleAIAction("Tests", "Du bist ein QA Engineer. Schreibe ausführliche Unit-Tests (Jest/Mocha Stil) für den folgenden Code. Gib NUR den vollständigen Test-Code zurück (kein Markdown drumherum, nur Raw Code).", { suffix: ".test", newExtension: ext });
