...
export const ArchitectPanel: React.FC = () => {
  ...
  const handleMassPush = async () => {
    ...
    try {
      ...
      logToSystem(`🟢 <b>GITOPS SUCCESS!</b><br>Alle ${batchFiles.length} Dateien wurden in einem einzigen Commit gepusht! Dein VPS zieht sich die Updates gleich.`, "success");
      setBatchFiles([]);
      setCommitMsg("");
      fetchRepoTree();
  ...
  const handleAIAction = async (action: string, sysPrompt: string, options: { isCodeUpdate?: boolean, newExtension?: string, outputFilename?: string, customContext?: string } = {}) => {
    ...
    try {
      const contextPrefix = options.customContext ? `${options.customContext}\n\n` : "";
      let response = await callGeminiAPI(`${contextPrefix}Datei: ${currentFile}\n\nCode:\n${currentFileContent}`, sysPrompt);
                ...
                <>
                  ...
                  <button onClick={() => {
                      ...
                      if (lang) {
                        ...
                        const parts = currentFile.split('.');
                        if (parts.length > 1) parts.pop();
                        const outName = parts.join('.') + `_translated.${newExt}`;
                        handleAIAction("Translate", `Du bist ein Expert Developer. Übersetze den folgenden Code in ${lang}. Erhalte die exakte Logik bei. Gib NUR den reinen übersetzten Code zurück, ohne Markdown-Blöcke.`, { outputFilename: outName });
                      }
                  ...
                  <button onClick={() => {
                      const parts = currentFile.split('.');
                      if (parts.length > 1) parts.pop();
                      handleAIAction("Diagram", "Du bist ein Software-Architekt. Erstelle ein Mermaid.js Diagramm (z.B. Flowchart oder Class Diagram), das den Ablauf und die Struktur des folgenden Codes visuell darstellt. Gib NUR den Mermaid-Code zurück, eingebettet in einem Markdown-Block: ```mermaid\\n[DEIN CODE]\\n```", { outputFilename: parts.join('.') + '-diagram.md' });
                  ...
                  <button onClick={() => {
                      const parts = currentFile.split('.');
                      if (parts.length > 1) parts.pop();
                      handleAIAction("Mock Data", "Du bist ein Backend-Entwickler. Analysiere den Code (Modelle, Interfaces, Variablen oder UI-Komponenten) und generiere dazu passende, extrem realistische Mock-Daten als JSON-Array mit 5 detaillierten Objekten. Gib AUSSCHLIESSLICH das nackte JSON zurück, ohne Markdown-Fences drumherum.", { outputFilename: parts.join('.') + '_mock.json' });
                  }} className="shrink-0 text-[10px] bg-indigo-100 text-indigo-700 px-2 py-1 rounded hover:bg-indigo-200 font-bold shadow-sm transition-all">✨ Mock Data</button>
                  ...
                  <button onClick={() => {
                      const parts = currentFile.split('.');
                      if (parts.length > 1) parts.pop();
                      handleAIAction("i18n", "Du bist ein Frontend-Architekt. Analysiere den Code und extrahiere alle hartcodierten, nutzersichtbaren Texte (Strings). Generiere eine JSON-Datei mit Key-Value-Paaren (Keys in UPPER_SNAKE_CASE, Values sind die Originaltexte). Gib AUSSCHLIESSLICH das validierte JSON zurück, absolut kein Markdown drumherum.", { outputFilename: parts.join('.') + '_i18n.json' });
                  }} className="shrink-0 text-[10px] bg-yellow-100 text-yellow-700 px-2 py-1 rounded hover:bg-yellow-200 font-bold shadow-sm transition-all">✨ i18n</button>
                  <button onClick={() => {
                      const parts = currentFile.split('.');
                      if (parts.length > 1) parts.pop();
                      handleAIAction("cURL API", "Du bist ein API-Spezialist. Analysiere diesen Code auf API-Endpunkte (z.B. REST-Routen, Controller, Fetch-Aufrufe). Generiere ein strukturiertes Markdown-Dokument mit realistischen, kopierbaren `curl`-Befehlen zum Testen dieser Endpunkte (inklusive nötiger JSON-Bodys und Headers). Gib NUR Markdown zurück, ohne Fences drumherum.", { outputFilename: parts.join('.') + '_endpoints.md' });
                  }} className="shrink-0 text-[10px] bg-cyan-100 text-cyan-700 px-2 py-1 rounded hover:bg-cyan-200 font-bold shadow-sm transition-all">✨ cURL API</button>
                  ...
                  <button onClick={() => {
                      const paths = fullTree.slice(0, 100).map(f => f.path).join('\n');
                      handleAIAction("Dockerize", "Du bist ein DevOps Engineer. Analysiere die Dateipfade des Repositories und den Code der aktuell geöffneten Datei, um den Tech-Stack zu erraten. Generiere ein professionelles, multi-stage, produktionsbereites 'Dockerfile'. Gib AUSSCHLIESSLICH den Inhalt des Dockerfiles zurück, ohne Markdown-Fences drumherum.", { outputFilename: "Dockerfile", customContext: `Dateibaum:\n${paths}` });
                  ...
                  <button onClick={() => {
                      const parts = currentFile.split('.');
                      if (parts.length > 1) parts.pop();
                      handleAIAction("SQL Schema", "Du bist ein Database Administrator. Analysiere den Code (Interfaces, Klassen, Structs, ORM-Modelle, JSON-Daten) und generiere das entsprechende SQL-Schema (PostgreSQL) mit CREATE TABLE Statements, korrekten Datentypen, Primary Keys und Foreign Keys. Gib AUSSCHLIESSLICH den rohen SQL-Code zurück, ohne Markdown-Fences drumherum.", { outputFilename: parts.join('.') + '_schema.sql' });
                  }} className="shrink-0 text-[10px] bg-amber-100 text-amber-700 px-2 py-1 rounded hover:bg-amber-200 font-bold shadow-sm transition-all">✨ SQL Schema</button>
                  <button onClick={() => {
                      const parts = currentFile.split('.');
                      if (parts.length > 1) parts.pop();
                      handleAIAction("TS Types", "Du bist ein TypeScript-Experte. Analysiere das übergebene JSON, JavaScript oder die ungetypten Strukturen und generiere strikte, professionell benannte TypeScript Interfaces und Types dafür. Gib AUSSCHLIESSLICH den rohen TypeScript-Code zurück, ohne Markdown-Fences drumherum.", { outputFilename: parts.join('.') + '_types.ts' });
                  }} className="shrink-0 text-[10px] bg-lime-100 text-lime-700 px-2 py-1 rounded hover:bg-lime-200 font-bold shadow-sm transition-all">✨ TS Types</button>
                  <button onClick={() => {
                      const paths = fullTree.slice(0, 100).map(f => f.path).join('\n');
                      handleAIAction("CI/CD", "Du bist ein DevOps Architekt. Analysiere den Dateibaum, um den Tech-Stack zu erkennen. Schreibe eine professionelle GitHub Actions CI/CD Pipeline (`main.yml`), die den Code auscheckt, Abhängigkeiten installiert, Tests ausführt und das Projekt baut. Gib AUSSCHLIESSLICH den rohen YAML-Code zurück, ohne Markdown-Fences.", { outputFilename: ".github/workflows/main.yml", customContext: `Dateibaum:\n${paths}` });
                  ...
                  <button onClick={() => {
                      const parts = currentFile.split('.');
                      const ext = parts.pop();
                      handleAIAction("Tests", "Du bist ein QA Engineer. Schreibe ausführliche Unit-Tests (Jest/Mocha Stil) für den folgenden Code. Gib NUR den vollständigen Test-Code zurück (kein Markdown drumherum, nur Raw Code).", { outputFilename: parts.join('.') + '.test.' + ext });
