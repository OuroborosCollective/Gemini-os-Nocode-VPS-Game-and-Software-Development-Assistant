...
export const ArchitectPanel: React.FC = () => {
  ...
  const runArchitect = async () => {
    ...
    try {
      ...
      for (let i = 0; i < plan.length; i++) {
        ...
        setIsLoadingFile(false);
        logToSystem(`✅ <code>${step.path}</code> fertiggestellt.`, "success", "system");
      }

      setCommitMsg(`Architect Deploy: Module Update (${batchFiles.length} files)`);
      setActiveTab("chat");
      logToSystem(`🎉 <b>Projekt-Modul lokal generiert!</b><br>Gib oben deinen PAT ein und klicke auf 'API MASS PUSH', um das komplette Projekt live zu schalten.`, "success", "system");
  ...
  const handleMassPush = async () => {
    ...
    try {
      ...
      logToSystem(`🟢 <b>GITOPS SUCCESS!</b><br>Alle ${batchFiles.length} Dateien wurden in einem einzigen Commit gepusht! Dein VPS zieht sich die Updates gleich.`, "success", "system");
      setBatchFiles([]);
      setCommitMsg("");
  ...
  const handleExpandBlueprint = async () => {
    ...
    try {
      ...
      logToSystem("✨ Blueprint erfolgreich detailliert.", "success", "bot");
    } catch (e: any) {
          ...
          <div className="h-16 border-t border-purple-200 px-4 flex items-center justify-between bg-purple-50 shrink-0 gap-4">
            <div className="truncate flex-1">
              ...
              <p className="text-[10px] text-purple-600 italic truncate mb-1">
                {batchStatus || `${batchFiles.length} Dateien bereit für Massen-Commit.`}
              </p>
